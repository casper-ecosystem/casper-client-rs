...
use casper_client::cli::{transaction_str_params::TransactionStrParams, CliError};
...
impl ClientCommand for MakeTransaction {
    ...
    async fn run(matches: &ArgMatches) -> Result<Success, CliError> {
        ...
        let payment_amount = creation_common::arg_simple::payment::get(matches);
        let force = common::force::get(matches);

        let transaction_kind = creation_common::transaction_kind::get(matches);
        let transaction_kind_args = creation_common::transaction_kind_args::get(matches);
